...
import { processPaymentCapture, extractPaymentDetailsFromResult } from '../utils/payment_helpers'; // For capturing payment on 'Black' status
...
export const updateOrderStatus = functions.https.onCall(
    ...
    async (request): Promise<{ success: true } | { success: false; error: string; errorCode: string }> => {
        const functionName = "[updateOrderStatus V1]";
        ...
        try {
            ...
            // 6. Handle Payment Capture on Completion
            if (newStatus === OrderStatus.Black && orderData.paymentStatus === PaymentStatus.Authorized) {
                ...
                const amountToCapture = orderData.finalAmount;
                ...
                if (!authId) {
                 ...
                 if (amountToCapture == null || amountToCapture <= 0) {
                     ...
                     updatedPaymentStatus = PaymentStatus.Paid;
                 } else {
                     ...
                     paymentDetailsUpdate = extractPaymentDetailsFromResult(captureResult);
                     ...
                     if (!captureResult.success) {
                         ...
                         updatedPaymentStatus = PaymentStatus.Captured;
            ...
            await db.runTransaction(async (transaction) => {
                ...
                const updateData: { [key: string]: any } = {
                    ...
                    statusHistory: FieldValue.arrayUnion({
                        status: newStatus, timestamp: now, userId: userId, role: userRole,
