...
enum ErrorCode {
    ...
    CurrencyMismatch = "CURRENCY_MISMATCH", // Added
}
...
export const createRentalBooking = functions.https.onCall(
    ...
    async (request): Promise<{ success: true; bookingId: string; requiresAction?: boolean; actionUrl?: string } | { success: false; error: string; errorCode: string }> => {
        const functionName = "[createRentalBooking V3 - Permissions]"; // Updated version name
        ...
        try {
            ...
            if (boxData.currencyCode !== rentalItemData.currencyCode) {
                 ...
                 throw new HttpsError('failed-precondition', `error.rental.currencyMismatch`, { errorCode: ErrorCode.CurrencyMismatch }); // Use specific error code
            }

            // 4. Permission Check (Using REAL helper)
            // Pass fetched role to checkPermission
            const hasPermission = await checkPermission(customerId, userRole, 'rental:create', logContext);
            if (!hasPermission) {
                logger.warn(`${functionName} Permission denied for user ${customerId} (Role: ${userRole}) to create rental booking.`, logContext);
                return { success: false, error: "error.permissionDenied.createRental", errorCode: ErrorCode.PermissionDenied };
            }

            // 5. Authorize Deposit Payment - Logic remains the same as V2
            ...
            authorizationResult = await initiateAuthorization(
                customerId, rentalItemData.depositSmallestUnit, rentalItemData.currencyCode,
            ...
            // 6. Firestore Transaction to Create Booking and Update Inventory - Logic remains the same as V2
            ...
            await db.runTransaction(async (transaction) => {
                ...
                const newBookingData: RentalBooking = {
                    ...
                    rentalFeeSmallestUnit: rentalItemData.rentalFeeSmallestUnit,
                    depositSmallestUnit: rentalItemData.depositSmallestUnit,
                    currencyCode: rentalItemData.currencyCode,
                    paymentStatus: paymentStatus,
                    paymentDetails: authDetails,
            ...
            // 7. Log User Activity (Async) - Logic remains the same as V2
            ...
            // 8. Return Success (potentially with action required) - Logic remains the same as V2
